...
def comp_turn():
    if mode == 'easy':
        ...
        if mode == 'hard':
            if not(matrix[4] == 'X' or matrix[4] == 'O'):
                matrix[4] = 'O'
            elif not(all_check()):
                a = random_in_need()
                if matrix[a] == 'X' or matrix[a] == 'O':
...
os.system("clear")
mode = raw_input("Which mode do you want to play in: Easy, Medium or Hard: ").lower()
while not(mode == 'easy' or mode == 'medium' or mode == 'hard'):
    os.system('clear')
    mode = raw_input('We cant understand your desired mode, retry: ').lower()
